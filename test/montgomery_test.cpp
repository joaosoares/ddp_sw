...
void customprint2(uint32_t *a, int size)
{
  int i;
  for (i = 0; i < size; i++)
  {
...
TEST_GROUP(MontgomeryTestGroup) {

};

TEST(MontgomeryTestGroup, MontFirstTest) {
  ...
  customprint2(res, 32);
  customprint2(expected, 32);
  int i;
  for (i = 0; i < 32; i++) {
    printf("expected is %08x, actual is %08x", expected[i], res[i]);
    CHECK_EQUAL(expected[i], res[i]); 
  }
}


IGNORE_TEST(MontgomeryTestGroup, MontSecondTest) {
...
IGNORE_TEST(MontgomeryTestGroup, MontThirdTest) {
...
TEST(MontgomeryTestGroup, AddCarryTest) {
